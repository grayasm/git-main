/* Cap.6 Structures, pag.139

   Suppose we want to handle the more general problem of counting the occurrences
   of all the words in some input. We can't do a linear search for each word as
   it arrives, to see if it's already been seen; the program would take too long.
   One solution is to keep the set of words seen so far sorted at all times.
   This sholdn't be done by shifting words ina linear array, though - that also
