...
GENERICAPI NTSTATUS GENERIC_EXPORT GenericEnableInterface(PGENERIC_EXTENSION pdx, const GUID* guid, BOOLEAN enable)
{
  ...
  /*	GenericGetDeviceCapabilities (added 2.0) returns the device capabilities.
...
GENERICAPI VOID GENERIC_EXPORT GenericSetDeviceState(PGENERIC_EXTENSION pdx, PNP_DEVICE_STATE pnpstate)
{
	...
	/*	Inform PnP manager that our state has canged.	*/
...
VOID AdjustDeviceCapabilities(PGENERIC_EXTENSION pdx, PDEVICE_CAPABILITIES pdc)
{
	...
	*/
	...
	/*	Make sure the device state for S5 is D3. Do this after adjusting wakeup
	capabilities because USBHUB uncritically assumes that a device can be
	in D2 from S1 through SystemWakeup. So if SystemWakeup happens to be S5
	because of some ACPI mistake, changing the device state entry too soon
	would cause us to pick D3 as the DeviceWake value, which is more wrong
...
NTSTATUS HandleQueryRemove(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	if (pdx->state == WORKING)
	{
		...
		the queue here instead of stalling it, by the way.
		*/
...
NTSTATUS HandleQueryState(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	to use conditional compilation to choose between 32- and 64-bit exchanges
	in the following statement.
	If the size of a PNP_DEVICE_STATE changes to be bigger than 32 bits,
	therefore, you have to change this by hand. The ASSERT will alert you to
	the problem...
	*/
...
NTSTATUS HandleRemoveDevice(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	/*	Disable all device interfaces that were registered through us.
	This triggers PnP notifications that will allow apps to close
	their handles.
	*/
...
NTSTATUS HandleStopDevice(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	we should already be in the PENDING state. There's a Win98 bug
	that can sometimes cause us to get a STOP instead of a REMOVE,
	in which case we should start rejecting IRPs.
	*/
...
NTSTATUS HandleUsageNotification(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	case DeviceUsageTypePaging:
	{
		...
		if (inpath && pdx->state != WORKING)
			return CompleteRequest(Irp, STATUS_DEVICE_NOT_READY); // SP-7, per DDK doc
																  ...
																  /*	Guard against reentrance by waiting on evPagingPath, which is
																  a synchronization event that was initialized to the signaled state.
		...
		/*	Add or subtract one from the paging path counter.
		AdjustSpecialFileCounter will also take care of enabling or
		disabling idle detection.
		*/

		AdjustSpecialFileCounter(pdx, inpath, &pdx->npagefiles);

		/*	Adjust the power-pagable flag too. (Per AdrianO) we must be
		careful not to have a non-paged power handler sitting on top
		of a paged handler, so we only clear DO_POWER_PAGABLE
		in our completion routine.
		*/
	...
	case DeviceUsageTypeDumpFile:
	  ...
	  /*	Install completion routine so we can recover from a failure and
	  so we can clear DO_POWER_PAGABLE at the right time.
	  */
...
NTSTATUS OnNotificationComplete(PDEVICE_OBJECT fdo, PIRP Irp, PGENERIC_EXTENSION pdx)
{
	...
	*/
	...
	/*	Somebody underneath us failed this usage notification,
	so undo the steps we took in the dispatch routine.
	Notice how we reverse the sense of "inpath" in the following calls
	to AdjustSpecialFileCounter.
	*/
