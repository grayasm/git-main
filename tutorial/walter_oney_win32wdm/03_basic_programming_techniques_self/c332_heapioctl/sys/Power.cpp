...
GENERICAPI NTSTATUS GENERIC_EXPORT GenericDispatchPower(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	if (fcn == IRP_MN_SET_POWER || fcn == IRP_MN_QUERY_POWER)
	{
		...
		/*	Create a context structure and launch the finite state machine
		that will process this IRP asynchronously.
		The initial call to HandlePowerEvent should return STATUS_PENDING.\
		The FSM will eventually complete the IRP.
		*/
...
GENERICAPI NTSTATUS GENERIC_EXPORT GenericHandlePowerIoctl(PGENERIC_EXTENSION pdx, PIRP Irp)
{
	...
	case IOCTL_SET_POWERINFO: // code == 0xFF1
	{
		...
		/*	If we're currently idle, restore power.
		This is in case we've just been told to never idle in the power mode
		the machine is currently in (which we can't easily determine).
		*/
...
GENERICAPI NTSTATUS GENERIC_EXPORT GenericWakeupControl(PGENERIC_EXTENSION pdx, enum WAKEFUNCTION wf)
{
	...
	manage:
	{
		if (pdx->WakeupEnabled)
		{
			...
			/*	If capabilities don't provide for wakeup, don't do anything.
			This often happens the first time we get called because the
			capabilities query hasn't happened yet.
			Also, we mustn't issue a WAIT_WAKE if the device is not in D0.
			*/
			...
			junk.SystemState = pdx->devcaps.SystemWake; // lowest state from which this IRP will wake system
														...
														/*	I have seen systems where SystemState has a ridiculous value,
														such that we have to ask for S0 or else it doesn't work...
														*/
	...
	case CancelWaitWake:
	{
		/*	If the following exchange operation returns NULL,
		WaitWakeComplete has run and the IRP may no longer exist.
		*/
		...
		if (Irp)
		{
			...
			/*	We need to be sure that IoCancelIrp and any cancel routine
			it may call finish before the Power Manager calls IoFreeIrp
			to get rid of the WAIT_WAKE IRP.
			The call to IoFreeIrp can't happen sooner than the IRP gets
			all the way through IoCompleteRequest, however, so we establish
...
VOID AdjustSpecialFileCounter(PGENERIC_EXTENSION pdx, BOOLEAN inpath, PLONG counter)
{
	...
	is just a macro that does exactly this...
	*/
	...
	Alter the not-disable-able status flag as well (SP-7, ver 1.6)
	*/
...
VOID WaitWakeCallback(PDEVICE_OBJECT junk, UCHAR MinorFunction, POWER_STATE state, PGENERIC_EXTENSION pdx, PIO_STATUS_BLOCK pstatus)
{
	...
	/*	Wakeup our own device.
	This is necessary if the device signaled a wakeup while the system
	was already in S0 because, in this case, we won't otherwise get
...
NTSTATUS WaitWakeCompletionRoutine(PDEVICE_OBJECT junk, PIRP Irp, PGENERIC_EXTENSION pdx)
{
	...
	/*	Nullify our WaitWakeIrp pointer to prevent anyone from dereferencing
...
NTSTATUS HandlePowerEvent(PPOWCONTEXT ctx, enum POWEVENT event)
{
	...
	*/
	...
	/*	Structurally, the following code is a switch on "action" embedded within an
	...
	while (TRUE)
	{
		...
		case TriageNewIrp:
		{
			...
			*/
			...
			/*	For a system IRP, we'll request the corresponding device IRP.
			If system power is being restored, we wait until the lower level drivers
			finish the system IRP. If system power is being removed,
			we do it now and forward the system IRP when the device IRP finishes.
			*/
			...
			/*	For a device set-power IRP, we have a variety of tasks to carry out.
			If device power is being restored, we do those tasks when the lower level drivers complete
			the IRP. If device power is being removed or staying the same, we do those tasks
			before passing this IRP down.
			In either case, we ensure that the device isn't busy with any substantive IRPs first.
			*/
			else
			{
				// device IRP
		...
		case ContextRestoreComplete:
		{
			...
			/*	If the device IRP failed, just go ahead and let it complete.
			If we've successfully resumed to a sleeping state (> D0), skip restarting the
		...
		case ContextSaveComplete:
		{
		  ...
		  kind of a query, by the way, but we should handle it nonetheless.
		  */
		...
		case RestoreSeqComplete:
		{
		  ...
		  /*	RestoreContext restores device context if necessary.
		  It can be entered from RestoreSeqComplete or from DevPowerUpComplete.
		  */
		case RestoreContext:
		{
		  ...
		  /*	InvalidAction is the action for any unexpected event.
