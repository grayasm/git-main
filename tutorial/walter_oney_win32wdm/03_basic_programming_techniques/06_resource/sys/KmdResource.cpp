...
NTSTATUS Win2KSetFilePosition(HANDLE handle, __int64 pos); // SetFilePosition
...
PVOID FindResourceEx(HANDLE hDriverResources, PWCHAR type, PWCHAR name, USHORT langid, ULONG& ressize)
{
	...
	/*
		Search for a version of the resource that best matches the requested language. 
		I coded this as a "while" so I could use a "break" when a match is found. 
		I modified the standard search rules by not including searches based on UI languages 
		because they depend on who's logged in. I think it's more important to provide 
		a consistent answer no matter when the driver happens to be loaded. 
		These other searches could be included after step 3 (call ZwQueryDefaultUILanguage) 
		and before step 4 (call ZwQueryInstallUILanguage).
	*/
...
NTSTATUS OpenDriverResourcesFromPathname(PUNICODE_STRING PathName, PHANDLE pHandle)
{
	...
	__try
	{
		...
		*/
		...
		status = Win2KSetFilePosition(hDriver, lfanew); // SetFilePosition
		if (!NT_SUCCESS(status))
			__leave;
		...
		status = Win2KSetFilePosition(hDriver, resoffset); // SetFilePosition
