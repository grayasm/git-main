...
/*
 * Thread start routine to "monitor" the counter. Every 3 seconds, try to
 * lock the mutex and read the counter. If the trylock fails, skip this cycle.
 */
void* monitor_thread(void* arg)
{
	...
	/* Loop until the_end, checking the counter every 3 seconds. */
	while( time(NULL) < end_time )
	{
		sleep(3);
	...
	printf("Monitor thread misses update %d times.\n", misses);
	return NULL;
}
...
int main(int argc, char** argv)
{
...
#ifdef sun
	/* On Solaris 2.5 (??), threads are not timesliced. To ensure that our
	 * threads can run concurrently, we need to increase the concurency
	 * level to 2.
	 */
	...
	status = pthread_create(&counter_thread_id,
	                        NULL,
	                        counter_thread,
	                        NULL);
	if(status)
	...
	status = pthread_create(&monitor_thread_id,
	                        NULL,
	                        monitor_thread,
	                        NULL);
	if(status)
