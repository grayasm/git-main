...
int main(int argc, char** argv)
{
	...
	if(status) err_abort (status, "init attr");
	...
	if(status) err_abort (status, "Get policy");
	status = pthread_attr_getschedparam (
		&thread_attr, &thread_param);
	if(status) err_abort (status, "Get sched param");
	...
	if(status) printf ("Unable to set SCHED_RR policy.\n");
	else {
		/* Just for the sake of the exercise, we'll use the
		 * middle of the priority range allowed for SCHED_RR.
		 * This should ensure that the thread will be run, without
		 * blocking everything else. Because any assumptions about
		 * how a thread's priority interacts with other threads
		 * (even in other processes) are nonportable, especially on
		 * an implementation that defaults to System contention scope
		 * you may have to adjust this code before it will work on
		 * some systems.
		 */
		rr_min_priority = sched_get_priority_min (SCHED_RR);
		if (rr_min_priority == -1) errno_abort ("Get SCHED_RR min priority");
		rr_max_priority = sched_get_priority_max (SCHED_RR);
		if (rr_max_priority == -1) errno_abort ("Get SCHED_RR max priority");
		...
		if(status) err_abort(status, "Set params");

		printf("Creating thread at RR/%d\n", thread_param.sched_priority);

		status = pthread_attr_setinheritsched (
			&thread_attr, PTHREAD_EXPLICIT_SCHED);
		if(status) err_abort(status, "Set inherit");
	}
#else
