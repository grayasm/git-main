...
void* tty_server_routine(void* arg)
{
    ...
    while(1)
    {
        ...
        {
            status = pthread_cond_wait(&tty_server.request,
                                       &tty_server.mutex);
            if(status != 0)
        ...
        if(request->synchronous)
        {
            ...
            if(status) err_abort(status, "Signal server condition");
...
void* client_routine(void* arg)
{
    ...
    if(client_threads <= 0)
    {
        ...
        if(status) err_abort(status, "Signal clients done");
    }
...
int main(int argc, char** argv)
{
    ...
    while(client_threads > 0)
    {
        ...
        if(status) err_abort(status, "Wait for clients to finish");
    }
    status = pthread_mutex_unlock(&client_mutex);
    if(status) err_abort(status, "Unlock client mutex");
