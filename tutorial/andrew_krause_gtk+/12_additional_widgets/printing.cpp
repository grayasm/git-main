...
void print_file  (GtkButton*, Widgets*);
void begin_print (GtkPrintOperation*, GtkPrintContext*, Widgets*);
void draw_page   (GtkPrintOperation*, GtkPrintContext*, gint, Widgets*);
...
int main (int argc, char** argv)
{
	...
	GtkWidget* print = gtk_button_new_from_stock (GTK_STOCK_PRINT);
	g_signal_connect (G_OBJECT (print),
	                  "clicked",
	                  G_CALLBACK (print_file),
	                  (gpointer) w);
...
void print_file  (GtkButton* button, Widgets* w)
{
	...
	*/
	...
	/*  Create a new print operation, applying saved print settings
	    if they exist.
	*/
	...
	g_signal_connect (G_OBJECT (operation),
	                  "begin_print",
	...
	{
		GtkWidget* dialog =
			gtk_message_dialog_new (GTK_WINDOW (w->window),
                  ...
                  Widgets* w)
{
	...
	/* Based on the height of the page and font size,
	   calculate how many lines can be rendered on a single page.
	   A padding of 3 is placed between lines as well.
	*/
	gdouble height =
		gtk_print_context_get_height (context) - HEADER_HEIGHT - HEADER_GAP;
...
/* Draw the page, which includes a header with the file name and
   page number along with one page of text with a font of "Monospace 10". */
                ...
                Widgets* w)
{
	...
	gint text_width;
	gchar *page_str = g_strdup_printf ("%d of %d", page_nr + 1, w->data->total_pages);
