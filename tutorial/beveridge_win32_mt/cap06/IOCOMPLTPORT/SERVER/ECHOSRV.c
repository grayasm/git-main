 ...
 *
 * Demonstrates how to use I/O completion ports with TCP
 * on the Internet. This sample server can only be run on
 * Windows NT, version 3.51 or later. The client (EchoCli)
 * can be run on Windows 95 or later.
 */
...
/* The context key keeps track of how the I/0
...
int main(int argc, char *argv[])
{
	 ...
	 *	socket (listener) to the I/0 completion
	 *	port!
	 */
	...
	if(ghCompletionPort == NULL)
		FatalError("CreateIoComepletionPort() failed");
	...
	fprintf(stderr, "Echo Server with I/0 Completion Ports\n");
...
DWORD WINAPI ThreadFunc(LPVOID pVoid)
{
	...
	// Loop forever on getting packets from the I/0 completion port
	for(;;)
	{
		...
		else if(bResult == FALSE && lpOverlapped != NULL)
		{
			...
			free(pCntx);
			fprintf(stderr, "ThreadFunc - I/0 operation failed\n");
		}
		else if(dwNumRead == 0)
		{
			closesocket(pCntx->sock);
		...
		// all back out (echo it) if we have seen a \n
		else
		{
 ...
 *	on a socket. Make sure we handle errors that
 *	are recoverable.
 */
void IssueRead(struct ContextKey *pCntx)
{
	...
	while(++i)
	{
		...
		// It succeeded immediately, but do not process it here,
		// wait for the completion packet.
		if(bResult)
			return;
