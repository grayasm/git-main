...
namespace lex
{
	...
	//False: negation of rule. It does not advance the stream;
	class False : public Base
	{
	public:
		typedef misc::autoptr<lex::False> Ptr;		

		~False();
		False(const Base::Ptr& rule);
		False(const False& tc);
		False& operator=(const False& tc);
