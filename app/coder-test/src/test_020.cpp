...
void test20::run(const char* test)
{
	test20 obj;
...
void test20::SetGrammar(ebnf::DefaultGrammar::Ptr& grammar)
{
	...
	m["A_F"] = new lex::Range('A', 'F');
	m["hexadecimal-digit"] = LEX(m["0_9"] | m["a_f"] | m["A_F"], TA_HEXADECIMAL, "not a hex digit", 1);
	...
	m["x"] = LEX(new lex::Integer('x'), TA_HEX_PREFIX, "expected x or X or an octal digit 0 to 7", 1);
	m["X"] = new lex::Integer('X');
	m["hexadecimal-literal"] = 
		LEX(
		(m["0"] & m["x"] & m["hexadecimal-digit"] & 0*m["hexadecimal-digit"]) |
		(m["0"] & m["X"] & m["hexadecimal-digit"] & 0*m["hexadecimal-digit"]),
		TA_HEXADECIMAL, "expected hex digit", 1)
		;
	...
	m["octal-literal"] = LEX(m["0"] & 0*m["octal-digit"], TA_OCTAL, "not an octal digit", 1);


	//	[decimal-literal]:
	//	nonzero-digit
	//	decimal-literal digit		
	m["decimal-literal"] = LEX((m["nonzero-digit"] & 0*m["0_9"]), TA_DECIMAL, "not a decimal digit", 1);
	...
	m["unsigned-suffix"] = 
		LEX(
		(m["u"] | m["U"]),
		TA_UNSIGNED_SUFFIX, "expected unsigned suffix u or U", 1)
		;
	...
	m["long-suffix"] = 
		LEX(
		m["l"] | m["L"],
		TA_LONG_SUFFIX, "expected long suffix l or L", 1)
		;
	...
	m["long-long-suffix"] = 
		...
		(m["l"] & m["l"]) |
		(m["L"] & m["L"]) ,
		TA_LONG_LONG_SUFFIX, "expected long long suffix ll or LL", 1)
		;
...
void test20::SetSyntax(lex::Base::Syntax& ref)
{	
...
void test20::SetReports(lex::Base::Reports& reps)
{
	lex::Report rep[16];
	rep[0].report(lex::Error("expected x or X or an octal digit 0 to 7", 1), 236, 6, 2);
	rep[1].report(lex::Error("expected x or X or an octal digit 0 to 7", 1), 240, 7, 2);
	rep[2].report(lex::Error("not a hex digit", 1), 250, 9, 3);
	rep[3].report(lex::Error("expected ;", 1), 254, 10, 3);
	rep[4].report(lex::Error("expected ;", 1), 332, 18, 8);
	rep[5].report(lex::Error("expected ;", 1), 342, 19, 8);
	rep[6].report(lex::Error("expected x or X or an octal digit 0 to 7", 1), 346, 20, 2);
	rep[7].report(lex::Error("not a hex digit", 1), 387, 23, 3);
	rep[8].report(lex::Error("expected ;", 1), 396, 24, 5);
	rep[9].report(lex::Error("expected ;", 1), 406, 26, 3);
	rep[10].report(lex::Error("not a hex digit", 1), 414, 27, 3);
	rep[11].report(lex::Error("expected ;", 1), 488, 35, 8);
	rep[12].report(lex::Error("expected ;", 1), 498, 36, 8);
	rep[13].report(lex::Error("expected x or X or an octal digit 0 to 7", 1), 502, 37, 2);
	rep[14].report(lex::Error("not a hex digit", 1), 543, 40, 3);
	rep[15].report(lex::Error("expected ;", 1), 552, 41, 5);
...
void test20::AddAsProduction(lex::Base::Syntax& ref, const char* p[], const int a[], int lim)
{	
