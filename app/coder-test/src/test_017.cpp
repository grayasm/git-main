...
void test17::run(const char* test)
{
	test17 obj;
...
void test17::SetGrammar(ebnf::DefaultGrammar::Ptr& grammar)
{
	grammar->SetTerm("endl");
	ebnf::Grammar::Map& m = grammar->GetMap();

	//grammar	
	//the Discard in this composition does not apply attribute. It cannot.
	...
	m["start"] = 
		...
		&&	~m["gaps"] //without effect on Discard
		&&	LEX(m["Circle"], TA_CIRCLE) //preserve lex::Error on m["Circle"
		&&	~m["gaps"] 
		&&	LEX(m["varname"], TA_VARNAME, "expected variable name", 1);
};


void test17::SetSyntax(lex::Base::Syntax& ref)
{	
	//"long \n"
	//"long long \n"
	//"long long long\n"
	//"long long long Circle __circle\n"
	const char* p1[]={"longlonglong","Circle","__circle"};//gaps discarded
	const int a1[]={TA_EXPRESSION, TA_CIRCLE, TA_VARNAME};
	...
	//"Long long long Circle __circle\n"
	//"long long long \tCircle __circle\n"
	const char* p2[]={"longlonglong","Circle","__circle"};//gaps discarded
	const int a2[]={TA_EXPRESSION, TA_CIRCLE, TA_VARNAME};
...
void test17::SetReports(lex::Base::Reports& reps)
{
...
void test17::AddAsProduction(lex::Base::Syntax& ref, const char* p[], const int a[], int lim)
{	
