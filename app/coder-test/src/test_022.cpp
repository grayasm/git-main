...
void test22::run(const char* test)
{
	test22 obj;
...
void test22::SetGrammar(ebnf::DefaultGrammar::Ptr& grammar)
{
	...
	//[s-char]:
	m["s-char"] = 
		LEX((m["0_255"] - (m["\""] | m["\\"] | m["endl"])), TA_INVALID_SEQ, "Tokens: \" \\ and new-line-character cannot be quoted.", 1) |
		(m["escape-sequence"]) |
	...
	m["start"] = 
			~m["gaps"]
		&&	LEX(m["string-literal"], TA_STRING_LITERAL, "invalid string literal", 1)
...
void test22::SetSyntax(lex::Base::Syntax& ref)
{	
...
void test22::SetReports(lex::Base::Reports& reps)
{
	lex::Report rep[4];
	rep[0].report(lex::Error("expected ;", 1), 1212, 80, 3);
	rep[1].report(lex::Error("invalid string literal", 1), 1215, 81, 1);
	rep[2].report(lex::Error("invalid string literal", 1), 1220, 82, 1);
	rep[3].report(lex::Error("invalid string literal", 1), 1222, 83, 1);
	for(int i=0; i<4; ++i)
...
void test22::AddAsProduction(lex::Base::Syntax& ref, const char* p[], const int a[], int lim)
{	
