...
/* The "alarm" structure now contains the time_t (time since the Epoch,
 * in seconds) for each alarm, so that they can be sorted. Storing the
 * requested number of seconds would not be enough, since the "alarm thread"
 * cannot tell how long it has been on the list.
 */
...
void* alarm_thread(void* arg)
{
	 ...
	 * disintegrated when the process exists. Lock the mutex at
	 * the start -- it will be unlocked during condition waits,
