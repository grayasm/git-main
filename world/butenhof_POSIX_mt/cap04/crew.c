 ...
 * It cnotains the "identity" of each worker.
 */
...
void* worker_routine(void* arg)
{
    ...
    if(!entry) errno_abort("Allocating dirent");

    status = pthread_mutex_lock(&crew->mutex);
    if(status) err_abort(status, "Lock crew mutex");
    ...
    while(crew->work_count == 0)
    {
        ...
        if(status) err_abort(status, "Wait for go");
    }
    ...
    while(1)
    {
        ...
        {
            status = pthread_cond_wait(&crew->go, &crew->mutex);
            if(status) err_abort(status, "Wait for work");
        }
        ...
        if(crew->work_count <= 0)
        {
            ...
            if(status) err_abort(status, "Wake waiters");
...
int crew_create(crew_t* crew, int crew_size)
{
    ...
    for(crew_index = 0; crew_index < CREW_SIZE; crew_index++)
    {
        ...
        if(status) err_abort(status, "Create worker");
    }
    return 0;
}
...
int crew_start(crew_p crew, char* filepath, char* search)
{
    ...
    status = pthread_mutex_unlock(&crew->mutex);
    if(status) err_abort(status, "Unlock crew mutex");
    return 0;
}
...
int main(int argc, char** argv)
{
...
#ifdef sun
    /* On Solaris 2.5, threads are not timesliced. To ensures
     * that our threads can run concurently, we need to
    ...
    if(status) err_abort(status, "Create crew");

    status = crew_start(&my_crew, argv[2], argv[1]);
    if(status) err_abort(status, "Start crew");
