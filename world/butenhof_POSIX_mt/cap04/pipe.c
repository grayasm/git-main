/* This programs describes a pipeline. */
 ...
 * pipeline. One for each thread, plus a "result stage"
 * where the final thread can stash the value.
 */
...
void* pipe_stage(void* arg)
{
	...
	status = pthread_mutex_lock(&stage->mutex);
	if(status) err_abort(status, "Lock pipe stage");
	while(1)
	{
		while(stage->data_ready != 1)
		{
			status = pthread_cond_wait(&stage->avail, &stage->mutex);
			if(status) err_abort(status, "Wait for previous stage");
		}

		pipe_send(next_stage, stage->data + 1);
		stage->data_ready = 0;
		status = pthread_cond_signal(&stage->ready);
		if(status) err_abort(status, "Wake next stage");
	}
...
int pipe_create(pipe_t* pipe, int stages)
{
	...
	if(status) err_abort(status, "Init pipe mutex");
	...
	for(pipe_index = 0; pipe_index <= stages; pipe_index++)
	{
		...
		if(new_stage == NULL) errno_abort("Allocate stage");

		status = pthread_mutex_init(&new_stage->mutex, NULL);
		if(status) err_abort(status, "Init stage mutex");

		status = pthread_cond_init(&new_stage->avail, NULL);
		if(status) err_abort(status, "Init avail condition");

		status = pthread_cond_init(&new_stage->ready, NULL);
		if(status) err_abort(status, "Init ready condition");
	...
	*link = (stage_t*)NULL;    /* Terminate list */
	     ...
	     stage = stage->next)
	{
		...
		if(status) err_abort(status, "Create pipe stage");
	}
	return 0;
}
 ...
 * pipe_results return to collect the final stage values
...
int pipe_start(pipe_t* pipe, long value)
{
	...
	if(status) err_abort(status, "Lock pipe mutex");

	pipe->active++;

	status = pthread_mutex_unlock(&pipe->mutex);
	if(status) err_abort(status, "Unlock pipe mutex");
...
int main(int argc, char** argv)
{
	...
	printf("Enter interger value, or \"=\" for next result\n");
