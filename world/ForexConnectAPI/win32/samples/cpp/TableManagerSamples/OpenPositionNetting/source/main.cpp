...
IO2GTradesTable *getTradesTable(IO2GTableManager *);
...
int main(int argc, char *argv[])
{
    ...
    if (bConnected)
    {
        ...
        if (account && strcmp(account->getMaintenanceType(), "0") == 0) // netting account
        {
            ...
            if (offer)
            {
                O2G2Ptr<IO2GTradesTable> tradesTable = getTradesTable(tableManager);
                if (tradesTable)
                {
...
IO2GTradesTable *getTradesTable(IO2GTableManager *tableManager)
{
    IO2GTradesTable *tradesTable = static_cast<IO2GTradesTable *>(tableManager->getTable(Trades));
    return tradesTable;
}
