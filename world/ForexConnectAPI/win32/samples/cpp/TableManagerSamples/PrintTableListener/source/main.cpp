...
{
    std::string procName = "PrintTableListener";
    if (argc == 1)
    {
        printHelp(procName);
        return -1;
    ...
    session->useTableManager(Yes, managerListener);
    ...
    if (bConnected)
    {
        ...
        {
            std::cout << "All tables are loaded" << std::endl;
            ...
            if (account)
            {
                ...
                O2G2Ptr<IO2GTableManager> tableManagerByAccount = session->getTableManagerByAccount(sampleParams->getAccount());
                if (managerListener->waitEvents() && managerListener->isLoaded())
                {
                    std::cout << "All tables are loaded (account " << sampleParams->getAccount() << ")" << std::endl;
                    printOrders(tableManagerByAccount, sampleParams->getAccount());
                }
                else
                {
                    std::cout << "Cannot refresh all tables of table manager (account " << sampleParams->getAccount() << ")" << std::endl;
                    bWasError = true;
                }
            }
            else
