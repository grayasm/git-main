...
void SessionStatusListener::onSessionStatusChanged(IO2GSessionStatus::O2GSessionStatus status)
{
    ...
    case IO2GSessionStatus::TradingSessionRequested:
    {
        ...
        if (descriptors)
        {
            ...
            std::cout << "Please enter trading session ID and press \'Enter\'" << std::endl;
            getline (std::cin, mSessionID);
            std::cout << "Please enter pin (if required). Then press \'Enter\'" << std::endl;
            getline (std::cin, mPin);
