...
void ResponseListener::setOrderIDs(std::vector<std::string> &orderIDs)
{
    mOrderIDs.resize(orderIDs.size());
    std::copy(orderIDs.begin(), orderIDs.end(), mOrderIDs.begin());
}
                ...
                for (int i = 0; i < reader->size(); ++i)
                {
                    if (reader->getUpdateTable(i) == Orders)
                    {
                        ...
                        if (reader->getUpdateType(i) == Update)
                        {
                            ...
                            iter = std::find(mOrderIDs.begin(), mOrderIDs.end(), order->getOrderID());
                            if (iter != mOrderIDs.end())
                            {
                                std::cout << "The order has been updated. OrderID='" << order->getOrderID() << "', "
                                        << "ContingentOrderID='" << order->getContingentOrderID() << "', "
                                        << "ContingencyType='" << order->getContingencyType() << "'"
                                        << std::endl;
                                mOrderIDs.erase(iter);
                                if (mOrderIDs.size() == 0)
                                    SetEvent(mResponseEvent);
                            }
                        }
