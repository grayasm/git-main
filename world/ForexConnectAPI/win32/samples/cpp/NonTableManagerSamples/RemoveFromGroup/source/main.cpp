...
IO2GRequest *removeOrdersFromGroupRequest(IO2GSession *, const char *, std::vector<std::string> &);
bool isOrderExists(IO2GSession *, const char *, const char *, ResponseListener *);

int main(int argc, char *argv[])
{
    std::string procName = "RemoveFromGroup";
    if (argc == 1)
    {
        printHelp(procName);
        return -1;
    ...
    if (bConnected)
    {
        ...
        if (account)
        {
            ...
            if (!bWasError)
            {
                O2G2Ptr<IO2GRequest> request = removeOrdersFromGroupRequest(session, sampleParams->getAccount(), orderIDs);
                if (request)
                {
...
IO2GRequest *removeOrdersFromGroupRequest(IO2GSession *session, const char *sAccountID, std::vector<std::string> &orderIDs)
{
