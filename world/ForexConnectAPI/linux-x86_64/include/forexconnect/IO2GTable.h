...
{
    IO2GTableIterator() : anchor(NULL), addref(NULL), release(NULL)
    {
        reset();
    }
    ...
    IO2GTableIterator(const IO2GTableIterator& other)
    {
        i = other.i;
        j = other.j;
        item = other.item;
...
class IO2GUpdatesProcessStatusListener : public IAddRef
{
 public:
    ...
    virtual void onInstrumentsUpdatesProcessStatusChanged(O2GUpdatesProcessStatus status) = 0;
};
