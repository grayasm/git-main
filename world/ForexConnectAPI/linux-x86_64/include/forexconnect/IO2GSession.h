...
class Order2Go2 IO2GSession : public IAddRef
{
 ...
 public:
    ...
    virtual bool login(const char *user, const char *pwd, const char *url, const char *connection) = 0;
    virtual bool loginWithToken(const char *user, const char *token, const char *url, const char *connection) = 0;
    ...
    */
    virtual int getReportURL(char* urlBuffer, int bufferSize, const char *accountID,
                             DATE dateFrom, DATE dateTo, const char* format, const char* langID) = 0;
    ...
    /** Gets token
        ...
        @return Number of characters written to the token buffer if the function succeeds and tokenBuffer is nonzero.
        The required size, in characters, for a buffer that can receive the token string if the function succeeds and tokenBuffer is zero.
        Negative value indicates failure. See O2GTokenError for possible values.
    */
    virtual int getToken(char *tokenBuffer, int bufferSize) = 0;
    ...
    /** Gets the commissions provider */
    virtual IO2GRolloverProvider* getRolloverProvider() = 0;
