...
void rectangular::rectangular_control()
{
    rectangular_primeste_datele();

    rectangular_registru();
};

void rectangular::rectangular_primeste_datele()
{
    ...
    for(int i=0; i < 3; ++i)
    {
        ...
        misc::cout << U("\n\t  x = ");
        g_control_lucru.control_lucru_getstdin(g_obiectiv.obiectiv_X_obiectiv[i]);
        
        misc::cout << U("\n\t  y = ");
        g_control_lucru.control_lucru_getstdin(g_obiectiv.obiectiv_Y_obiectiv[i]);
        
        misc::cout << U("\n\t  h = ");
        g_control_lucru.control_lucru_getstdin(g_obiectiv.obiectiv_h_obiectiv[i]);
...
void rectangular::rectangular_registru()
{
    FILE *f ;
    ...
    if(g_control_lucru.control_lucru_fisier_cumulat_P_CRez_C_O_D_Coef  == 1 && g_control_lucru.control_lucru_P_C == 1)
    {
        ...
        for(int i=0; i < 3; ++i)
        {
            ...
            fprintf(f, "    Indicarea obiectivului prin coordonate rectangulare\n");
            fprintf(f, "%s %s\n", "    Obiectiv = ",  g_obiectiv.obiectiv_nr_obiectiv[i]);
    ...
    if(g_control_lucru.control_lucru_fisier_cumulat_T_FRez_C_O_D_Coef  == 1 && g_control_lucru.control_lucru_T_F == 1)
    {
        ...
        for(int i=0; i < 3; i++)
        {
            ...
            if(g_control_lucru.control_lucru_intrerupe_calcul_depasire_limite[i] == 0)
            {
                ...
                if(g_obiectiv.obiectiv_norma_consum[i] == 0)
                {
                    fprintf(f, "%s", "   vijelie INCARCATI (FOC!)");
                }
                else if(g_obiectiv.obiectiv_norma_consum[i] > 0)
                {
                    fprintf(f, "%s %.1f\n", "    ritm  = ", g_obiectiv.obiectiv_ritm_de_tragere[i]);
