...
citire_distanta::citire_distanta()
{
};
...
void citire_distanta::citire_distanta_control()
{
    primeste_datele();
...
void citire_distanta::primeste_datele()
{
    ...
    for(int i=0; i < 3; ++i)
    {
        ...
        misc::cout << U("\n\t  Citire = ");
        g_control_lucru.control_lucru_getstdin(cCl, 10);
        ...
        if(g[i] < 1 || g[i] > 5)
        {
            misc::cout << U("\n\t  Eroare: sursa fata de care se indica obiectivul este necunoscuta.");
            getchar();
            exit(EXIT_FAILURE);
        }//else
        else if(g[i] < 4)
        {
            objPos = Util::polar(
                g_dispozitiv_de_lupta.dispozitiv_de_lupta_Xpco[g[i]-1], 
                g_dispozitiv_de_lupta.dispozitiv_de_lupta_Ypco[g[i]-1],
                g_dispozitiv_de_lupta.dispozitiv_de_lupta_hpco[g[i]-1], 
                Dap[i],
                To[i], 
                tct[i], 
...
void citire_distanta::afiseaza_datele()
{    
    for(int i=0; i< 3; i++)
    {        
        if(g_obiectiv.obiectiv_btno[i] == 0)
...
void citire_distanta::registru()
{
    ...
    if(g_control_lucru.control_lucru_fisier_cumulat_P_C_I_O == 1 && g_control_lucru.control_lucru_P_C == 1)
    {
        ...
        for(int i=0; i<3; i++)
        {
            ...
            if(g[i] < 4)
            {
                fprintf(f, "%s %d %s %.0f %s %.2f\n", " De Bateria ", g[i], " Citire = ", Cl[i], " Distanta = ", Dap[i]);
            }
            else if (g[i] == 4) // PO
            {
                fprintf(f, "%s %.0f %s %.2f\n", "   De Divizion (PO) Citire = ", Cl[i], " Distanta = ", Dap[i]);
            }            
            else if (g[i] == 5) // POL
            {
                fprintf(f, "%s %.0f %s %.2f\n", "   De Divizion (POL)  Citire = ", Cl[i], " Distanta = ", Dap[i]);
            }
            else
            {
    ...
    if(g_control_lucru.control_lucru_fisier_cumulat_P_CRez_C_O_D_Coef  == 1 && g_control_lucru.control_lucru_P_C == 1)
    {
        ...
        for(int i=0; i<3; i++)
        {
            ...
            fprintf(f, "    Indicarea obiectivului prin citire si distanta\n");
            fprintf(f, "%s %s\n", "    Obiectiv = ",  g_obiectiv.obiectiv_nr_obiectiv[i]);
