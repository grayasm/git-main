/*
 * Sample to calculate pi twice and display the amount of time
 * it takes to make each of the two calculations. The first run
 * uses a normal function call, so it works in the way you are
 * used to. The second run has a worker thread do the calculation
 * while the primary thread sits in a busy loop waiting for the 
 * worker thread to finish.
 *
 * Demonstrate the effect on performance of using a busy loop.
*/
