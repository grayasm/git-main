...
void entry_changed (GtkEditable*, GtkAssistant*);
void button_toggled (GtkCheckButton*, GtkAssistant*);
void button_clicked (GtkButton*, GtkAssistant*);
...
int main (int argc, char** argv)
{
	...
	page[0].widget = gtk_label_new ("This is an example of a GtkAssistant.\n"
	                                "By clicking the forward button, you can\n"
	...
	page[4].widget = gtk_label_new ("Text has been entered in the label and\n"
	                                "the combo box is clicked. If you are\n"
	                                "done, then it is time to leave!");
	...
	/* Create the necessary widgets for the fourth page. The attach the
	   progress bar to the GtkAlignment widget for later access. */
	...
	/* Update whether pages 2 through 4 are complete based upon whether there is
	   text in the GtkEntry, the check button is active ,or the progress bar is
	   completely filled. */
	g_signal_connect (G_OBJECT(entry),
	                  "changed",
	                  G_CALLBACK(entry_changed),
	                  (gpointer) assistant);
	g_signal_connect (G_OBJECT(page[2].widget),
	                  "toggled",
	...
	g_signal_connect (G_OBJECT(assistant),
	                  "cancel",
	                  G_CALLBACK(assistant_cancel),
	                  NULL);
	g_signal_connect (G_OBJECT(assistant),
	                  "close",
	                  G_CALLBACK(assistant_close),
	                  NULL);
...
void entry_changed (GtkEditable* entry, GtkAssistant* assistant)
{
...
void button_toggled (GtkCheckButton* toggle, GtkAssistant* assistant)
{
