...
void ok_clicked (GtkButton*, GtkWidget*);
void font_dialog_response (GtkFontSelectionDialog*, gint, gpointer);
...
/* If the user clicks "Apply", display the font, but do not destroy the dialog.
   If "OK" is pressed, display the font and destroy the dialog. Otherwise,
   just destroy the dialog. */
void font_dialog_response (GtkFontSelectionDialog* dialog,
                           gint response,
                           gpointer data)
{
	gchar* font;
	...
	case GTK_RESPONSE_OK:
	{
		...
		message = gtk_message_dialog_new (NULL,
		                                  ...
		                                  GTK_BUTTONS_OK,
		                                  font);
		gtk_window_set_title (GTK_WINDOW(message), "Selection Font");
