...
void my_ip_address_class_init (MyIPAddressClass*);
void my_ip_address_init (MyIPAddress*);
void my_ip_address_get_property (GObject*, guint, GValue*, GParamSpec*);
void my_ip_address_set_property (GObject*, guint, const GValue*, GParamSpec*);

void my_ip_address_render (MyIPAddress*);
void my_ip_address_move_cursor (GObject*, GParamSpec*);
gboolean my_ip_address_key_pressed (GtkEntry*, GdkEventKey*);

guint my_ip_address_signals[LAST_SIGNAL] = { 0 };
...
*/
void my_ip_address_class_init (MyIPAddressClass *klass)
{
...
*/
void my_ip_address_set_property (GObject *object,
                                 guint prop_id,
                                 const GValue *value,
                                 GParamSpec *pspec)
{
...
*/
void my_ip_address_get_property (GObject *object,
                                 guint prop_id,
                                 GValue *value,
                                 GParamSpec *pspec)
{
...
/*  Initialize the actual MyIPAddress widget.
    This function is used to setup the initial view of the widget and
    set necessary properties.
*/
void my_ip_address_init (MyIPAddress *ipaddress)
{
	MyIPAddressPrivate *priv = MY_IP_ADDRESS_GET_PRIVATE (ipaddress);
  ...
  /*  The key-press-event signal will be used to filter out certain keys.
      We will also monitory the cursor-position property so it can be
      moved correctly.
  */
...
/*  Set the content of the IP address.
    Ignore any values that are out of bounds so the programmer can
    provide a negative value for fields not to change.
*/
void my_ip_address_set_address (MyIPAddress *ipaddress,
                                gint address[4])
{
	MyIPAddressPrivate *priv = MY_IP_ADDRESS_GET_PRIVATE (ipaddress);
...
void my_ip_address_render (MyIPAddress *ipaddress)
{
	MyIPAddressPrivate *priv = MY_IP_ADDRESS_GET_PRIVATE (ipaddress);
...
void my_ip_address_move_cursor (GObject *entry, GParamSpec *spec)
{
...
gboolean my_ip_address_key_pressed (GtkEntry *entry, GdkEventKey *event)
{
