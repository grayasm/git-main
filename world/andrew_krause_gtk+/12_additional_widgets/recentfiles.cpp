...
void open_file (GtkButton*, Widgets*);
void save_file (GtkButton*, Widgets*);
void open_recent_file (GtkButton*, Widgets*);
void menu_activated (GtkMenuShell*, Widgets*);
...
int main (int argc, char** argv)
{
	...
	GtkWidget* icon = gtk_image_new_from_stock (GTK_STOCK_OPEN,
  ...
  /* Load the default recent chooser menu and create a menu from it. */
	...
	g_signal_connect (G_OBJECT (save),
	                  ...
	                  (gpointer) w);
	g_signal_connect (G_OBJECT (w->recent),
	                  "clicked",
	                  G_CALLBACK (open_recent_file),
	                  (gpointer) w);
...
void open_file (GtkButton* open, Widgets* w)
{
	gchar *groups[2] = { (gchar*)"testapp", (gchar*) NULL };
	...
	if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
	{
		...
		if (g_file_get_contents (filename, &content, &length, NULL))
		{
			...
			data->mime_type = (gchar*)"text/plain";
...
void save_file (GtkButton* save, Widgets* w)
{
	const gchar* filename = gtk_window_get_title (GTK_WINDOW(w->window));
...
void open_recent_file (GtkButton* recent, Widgets* w)
{
	GtkRecentManager* manager = gtk_recent_manager_get_default ();
	GtkWidget* dialog = gtk_recent_chooser_dialog_new_for_manager(
                                           "Open Recent File",
                                           GTK_WINDOW(w->window),
                                           manager,
                                           GTK_STOCK_CANCEL,
	...
	{
		gchar* filename =
			gtk_recent_chooser_get_current_uri (GTK_RECENT_CHOOSER (dialog));

		if (filename != NULL)
		{
			/* Remove the "file://" prefix from the beginning
			   of the URI if it exists.
			*/
			gchar* fn = g_filename_from_uri (filename, NULL, NULL);
			gchar* content;
			gsize  length;
...
void menu_activated (GtkMenuShell* menu, Widgets* w)
{

	gchar* filename =
		gtk_recent_chooser_get_current_uri (GTK_RECENT_CHOOSER (menu));
