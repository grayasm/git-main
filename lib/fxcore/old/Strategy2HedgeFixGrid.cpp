...
namespace fx
{
	Strategy2HedgeFixGrid::Strategy2HedgeFixGrid()
	{
		Reset();
	}
	...
	Strategy2HedgeFixGrid::Strategy2HedgeFixGrid(const Strategy2HedgeFixGrid& tc)
	{
		Reset();
		*this = tc;
	}

	Strategy2HedgeFixGrid& Strategy2HedgeFixGrid::operator=(const Strategy2HedgeFixGrid& tc)
	{
		if(this != &tc)
		{
			m_ep = tc.m_ep;
	...
	void Strategy2HedgeFixGrid::AcceptVisitor(StrategyVisitor& visitor)
	{
		visitor.Visit(*this);
	}

	bool Strategy2HedgeFixGrid::Update(const Price& rate)
	{			
	...
	double Strategy2HedgeFixGrid::GetPL() const
	{
		return m_ct.GetPL(m_ep.GetCurrency().GetSymbol(), m_rate);
	}

	double Strategy2HedgeFixGrid::GetGPL() const
	{
		return m_ct.GetGPL(m_ep.GetCurrency().GetSymbol(), m_rate);
	}

	bool Strategy2HedgeFixGrid::ReInit()
	{
	...
	misc::string Strategy2HedgeFixGrid::ToString() const
	{
	...
	void Strategy2HedgeFixGrid::Reset()
	{
	...
	void Strategy2HedgeFixGrid::Validate()
	{
		bool valid = true;
		if(m_dh <= 1 /*|| ceil(m_dh) != m_dh*/) // 164.2 deserialized to 1.64.1999
			valid = false;
		...
		// m_dh does not influence the initial hedge.
	...
	bool Strategy2HedgeFixGrid::CanEnterMarket()
	{
		if(!m_ct.IsEmpty())
			return false;
	...
	bool Strategy2HedgeFixGrid::EnterMarket()
	{
		// check orders
		if(!m_ct.IsEmpty())
			throw misc::exception("Orders added already.");
		
		// Enter with a Buy and a Sell based on entry position data.
		for(size_t i = 0; i < 2; ++i)
		{
			...
			misc::string error;
			if(!m_plugin->OpenMarketOrder(pos, orders, error))
				return false;
			...
			// remember we get back only the open price not both (bid/ask)
			double midprice = 0;
			for(size_t i = 0; i < orders.size(); ++i)
			{
				const fx::Position& order = orders[i];
				const fx::Price& price = order.GetCurrency().GetPrice();
				midprice += (isBuy == true ? price.GetBuy() : price.GetSell());
			}
			midprice /= orders.size();

			// set the entry for later reference
			double buy = (isBuy == true ? midprice : m_enter.GetBuy());
			double sell = (isBuy == true ? m_enter.GetSell() : midprice);
	...
	bool Strategy2HedgeFixGrid::CanCreateGrid()
	{
		return m_grid.empty();
	}

	bool Strategy2HedgeFixGrid::CreateGrid()
	{
		if(!m_grid.empty())
	...
	bool Strategy2HedgeFixGrid::UpdateGrid()
	{
	...
	bool Strategy2HedgeFixGrid::ShowStatus()
	{
	...
	bool Strategy2HedgeFixGrid::BlockTransaction()
	{
		double amount = 0;
		Transaction tro;
		...
		// check if excess is correct (from StrategyHedgeFixGrid)
	...
	bool Strategy2HedgeFixGrid::CloseTransaction()
	{
		bool ret = true;
