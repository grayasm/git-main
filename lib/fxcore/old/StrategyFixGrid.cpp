...
namespace fx
{
	StrategyFixGrid::StrategyFixGrid()
	{
		Reset();
	}

	StrategyFixGrid::StrategyFixGrid(const StrategyFixGrid& tc)
	{
		Reset();
		*this = tc;
	}

	StrategyFixGrid& StrategyFixGrid::operator=(const StrategyFixGrid& tc)
	{
		if(this != &tc)
		{
			m_ep = tc.m_ep;
	...
	void StrategyFixGrid::AcceptVisitor(StrategyVisitor& visitor)
	{
		visitor.Visit(*this);
	}

	bool StrategyFixGrid::Update(const Price& rate)
	{
		m_prev_rate = m_rate;
		m_rate = rate;
	...
	misc::string StrategyFixGrid::ToString() const
	{
	...
	bool StrategyFixGrid::CreateGrid()
	{
		if(!m_grid.empty())
			throw misc::exception("Grid must be empty.");

		/*	Create the grid from current m_rate quote, not from what was given
	...
	bool StrategyFixGrid::UpdateGrid()
	{
	...
	bool StrategyFixGrid::ShowStatus()
	{
	...
	bool StrategyFixGrid::BlockTransaction()
	{
		double amount = 0;
		Transaction tro;
	...
	bool StrategyFixGrid::CloseTransaction()
	{
		bool ret = true;
