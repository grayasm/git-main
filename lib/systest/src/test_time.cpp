...
void test_time::ctor()
{
	...
	sys::time t0;
	TEST_CHECK( t0.tostring() == U("1970-JAN-1 0:0:0") );
	...
	*/
	...
	TEST_CHECK( t1.tostring() == U("2014-MAR-30 2:0:0"));
	
	sys::time t2(2014, sys::time::OCT, 26, 3, 0, 0);
	TEST_CHECK( t2.tostring() == U("2014-OCT-26 3:0:0"));
	
	sys::time t3(2015, sys::time::MAR, 29, 2, 0, 0);
	TEST_CHECK( t3.tostring() == U("2015-MAR-29 2:0:0"));
	
	sys::time t4(2015, sys::time::OCT, 25, 3, 0, 0);
	TEST_CHECK( t4.tostring() == U("2015-OCT-25 3:0:0"));
	
	sys::time t5(2016, sys::time::MAR, 27, 2, 0, 0);
	TEST_CHECK( t5.tostring() == U("2016-MAR-27 2:0:0"));
	
	sys::time t6(2016, sys::time::OCT, 30, 3, 0, 0);
	TEST_CHECK( t6.tostring() == U("2016-OCT-30 3:0:0"));
	
	sys::time t7(2017, sys::time::MAR, 26, 2, 0, 0);
	TEST_CHECK( t7.tostring() == U("2017-MAR-26 2:0:0"));
	
	sys::time t8(2017, sys::time::OCT, 29, 3, 0, 0);
	TEST_CHECK( t8.tostring() == U("2017-OCT-29 3:0:0"));
	
	sys::time t9(2018, sys::time::MAR, 25, 2, 0, 0);
	TEST_CHECK( t9.tostring() == U("2018-MAR-25 2:0:0"));
	
	sys::time t10(2018, sys::time::OCT, 28, 3, 0, 0);
	TEST_CHECK( t10.tostring() == U("2018-OCT-28 3:0:0"));
...
void test_time::copy_ctor()
{
	...
	sys::time t0;
	sys::time t1(t0);
	TEST_CHECK( t1.tostring() == U("1970-JAN-1 0:0:0") );
	
	sys::time t2(2015, sys::time::APR, 25, 16, 42, 38);
	sys::time t3 = t2;
	TEST_CHECK( t3.tostring() == U("2015-APR-25 16:42:38") );
}
...
void test_time::sub_op()
{
	...
	TEST_CHECK ( illegalsub );
	
	sys::time t4 = t1 - t2;
	TEST_CHECK( t4.tostring() == U("1970-JAN-1 22:49:50") );
}
...
void test_time::add_op()
{
	...
	sys::time t3 = t1 + t2;
	TEST_CHECK( t3.tostring() == U("2014-MAR-30 2:0:0") );
}
...
void test_time::tolocaltime()
{
	...
	sys::time t0;
	TEST_CHECK( t0.tolocaltime() != U("1970-JAN-1 0:0:0") );
}
...
void test_time::year_()
{
	...
	*/
	...
	TEST_CHECK(t1.tostring() == U("2038-JAN-18 23:59:59"));
	t1 += sys::time::hourSEC;
	stl::string t1str = t1.tostring();
	TEST_CHECK(t1str == U("2038-JAN-19 0:59:59"));
}
...
void test_time::wday()
{
	...
	TEST_CHECK( sys::time::SAT == t1.wday() );
}
...
void test_time::isdst()
{
	...
	TEST_CHECK(t1.isdst() == 0);
	TEST_CHECK(t2.isdst() == 0); // should be zero, DST not set.
}
